...
import { signupAsync, selectSignupStatus, selectSignupError, clearSignupError, resetSignupStatus, selectLoggedInUser } from './AdminAuthSlice';
...
export const Signup = () => {
  ...
  useEffect(() => {
    if (status === 'fullfilled') {
      toast.success('Welcome! Verify your email to start selling on NeoBuy.');
      reset();
    }
    return () => {
      ...
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
        ...
        <div className="flex justify-between items-center mt-4 text-sm">
          <MotionConfig whileHover={{ x: 2 }} whileTap={{ scale: 1.05 }}>
            ...
            <motion.div>
              <Link to="/admin/login" className="text-blue-600 hover:underline">Already a member? <span className="font-semibold">Login</span></Link>
            </motion.div>
          </MotionConfig>
        </div>
      </div>
