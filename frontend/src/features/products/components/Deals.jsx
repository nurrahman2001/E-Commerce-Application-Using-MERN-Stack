...
export const Deals = () => {
    ...
    return (
        <div className="bg-white w-full mt-10 pb-10">
            <h2 className="text-2xl py-6 text-center">On Sale</h2>

            <div className="relative flex items-center justify-center px-16">
                {/* Left Button */}
                <button
                    className={`absolute left-0 z-10 p-2 rounded-full ${currentIndex === 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-300"
                        }`}
                    onClick={prevSlide}
                ...
                <div {...swipeHandlers} className="overflow-hidden w-full">
                    ...
                    >
                        {bestSellers.map((product) => (
                            <div
                                key={product._id}
                                className="flex-shrink-0 border border-gray-200 cursor-pointer shadow-md overflow-hidden"
                                style={{ width: `${itemWidth}px` }} // Set a fixed width
                                onClick={() => navigate(`/product-details/${product._id}`)}
                            >
                                <div className="relative">
                                    <span className='bg-red-600 text-gray-100 text-sm rounded-full px-3 top-3 right-6 absolute'>SALE</span>
                                </div>
                ...
                {/* Right Button */}
                <button
                    className={`absolute right-0 z-10 p-2 rounded-full ${currentIndex + 4 >= bestSellers.length ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-300"
                        }`}
                    onClick={nextSlide}
